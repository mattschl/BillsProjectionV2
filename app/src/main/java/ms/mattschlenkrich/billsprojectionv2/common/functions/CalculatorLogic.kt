...
object CalculatorLogic {
    ...
    fun formatFormula(
        ...
        if (operator.isEmpty()) return nf.getNumberFromDouble(currentValue)
        return "${nf.getNumberFromDouble(prevValue)} $operator " +
                "${nf.getNumberFromDouble(currentValue)} = ${nf.getDollarsFromDouble(result)}"
    }
}