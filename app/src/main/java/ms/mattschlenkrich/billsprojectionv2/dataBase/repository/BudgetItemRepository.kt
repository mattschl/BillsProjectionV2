...
class BudgetItemRepository(private val db: BillsDatabase) {
    ...
    suspend fun purgeOldBudgetItems(cutoffDate: String) =
        db.getBudgetItemDao().purgeOldBudgetItems(cutoffDate)

    fun getPayDaysActive() = db.getBudgetItemDao().getPayDaysActive()

    suspend fun deleteFutureItems(
        currentDate: String, updateTime: String
    ) = db.getBudgetItemDao().deleteFutureItems(
    ...
    fun getBudgetItemsAll(asset: String, payDay: String) =
        db.getBudgetItemDao().getBudgetItemsAll(asset, payDay)
